...
public class IntakeIOTalonFX implements IntakeIO {
  ...
  final Debouncer topIntakeConnectedDebounce = new Debouncer(0.5, Debouncer.DebounceType.kFalling);
  ...
  public void updateInputs(IntakeIOInputs inputs) {
    var topIntakeStatus =
        BaseStatusSignal.refreshAll(
            topIntakeAppliedVoltage, topIntakeCurrentAmps, topIntakeVelocity);

    var bottomIntakeStatus =
        BaseStatusSignal.refreshAll(
            bottomIntakeAppliedVoltage, bottomIntakeCurrentAmps, bottomIntakeVelocity);

    inputs.topIntakeConnected = topIntakeConnectedDebounce.calculate(topIntakeStatus.isOK());
    inputs.bottomIntakeConnected =
        bottomIntakeConnectedDebounce.calculate(bottomIntakeStatus.isOK());

    inputs.topIntakeVelocityRadsPerSec =
        Units.rotationsToRadians(topIntakeVelocity.getValueAsDouble());
    inputs.bottomIntakeVelocityRadsPerSec =
        Units.rotationsToRadians(bottomIntakeVelocity.getValueAsDouble());
